    ...
    <div style="border: 1px solid #e6e6e6;margin-bottom: 10px;padding:15px">
      <el-row :span="24">
        <el-col :span="2" style="padding-left: 6px; padding-right: 6px;">
          <div v-model="httpType" class="httpStyle">{{ httpType }}</div>
        </el-col>
        <el-col :span="2" style="padding-left: 6px;">
          <div v-model="requestType" class="httpStyle">{{ requestType }}</div>
        </el-col>
        ...
        <el-col :span="5">
          <i v-show="status" class="el-icon-check statusIcon" />
          <i v-show="!status" class="el-icon-close statusIcon" />
          <div v-model="updateTime" class="apiDate">{{ updateTime }}</div>
        </el-col>
      </el-row>
    </div>
    <el-collapse v-model="activeNames">
      <el-collapse-item title="请求头部" name="1">
        <el-table v-loading="listLoadingHead" :data="head" highlight-current-row style="width: 100%;">
      ...
      <el-collapse-item title="请求参数" name="2">
        <template>
          <div
            v-show="parameterRaw"
            v-model="parameterRaw"
            :class="ParameterTyep? 'parameter-b': 'parameter-a'"
            style="border: 1px solid #e6e6e6;margin-bottom: 10px;padding:15px;word-break: break-all"
          >{{ parameterRaw }}</div>
        </template>
        <div v-show="!parameter.length&&!parameterRaw" class="raw">暂无数据</div>
        <el-table
          v-loading="listLoadingParameter"
          :data="parameter"
      ...
      <el-collapse-item title="返回参数" name="3">
        <el-table v-loading="listLoadingResponse" :data="response" highlight-current-row style="width: 100%;">
          <el-table-column type="index" label="#" min-width="10%" sortable />
      ...
      <el-collapse-item title="普通Mock" name="4">
        <el-card class="box-card">
          ...
          <div
            v-show="mockData"
            v-model="mockData"
            :class="resultShow? 'parameter-a': 'parameter-b'"
            style="border: 1px solid #e6e6e6;margin-bottom: 10px;padding:15px;word-break: break-all;line-height:25px"
          >{{ mockData }}</div>
