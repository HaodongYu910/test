<template>
  <section>
    <router-link :to="{ name: '基础信息', params: {project_id: this.$route.params.project_id,api_id:this.$route.params.api_id}}" style="text-decoration: none;color: aliceblue;">
      <el-button class="return-list"><i class="el-icon-d-arrow-left" style="margin-right: 5px" />接口详情</el-button>
    </router-link>
    <router-link :to="{ name: '基础信息', params: {project_id: this.$route.params.project_id,api_id:this.$route.params.api_id}}" style="text-decoration: none;color: aliceblue;">
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="18">
            <el-form-item prop="addr">
              <el-input v-model.trim="form.addr" placeholder="地址" auto-complete />
            </el-form-item>
          </el-col>
        </el-row>
      </div>
        ...
        <el-collapse v-model="activeNames" @change="handleChange">
          <el-collapse-item title="请求头部" name="1">
            <el-table :data="form.head" highlight-current-row>
              ...
              <el-table-column label="" min-width="10%">
                <template slot-scope="scope">
                  <el-button v-if="scope.$index===(form.head.length-1)" size="mini" class="el-icon-plus" @click="addHead" />
                </template>
              </el-table-column>
            </el-table>
          </el-collapse-item>
          <el-collapse-item title="请求参数" name="2">
            ...
            <el-table :data="form.parameter" highlight-current-row :class="ParameterTyep? 'parameter-a': 'parameter-b'">
              ...
              <el-table-column label="" min-width="4%">
                <template slot-scope="scope">
                  <el-button v-if="scope.$index===(form.parameter.length-1)" size="mini" class="el-icon-plus" @click="addParameter" />
                </template>
              </el-table-column>
            </el-table>
            <template>
          ...
          <el-collapse-item title="返回参数" name="3">
            <el-table :data="form.response" highlight-current-row>
              ...
              <el-table-column label="" min-width="4%">
                <template slot-scope="scope">
                  <el-button v-if="scope.$index===(form.response.length-1)" size="mini" class="el-icon-plus" @click="addResponse" />
                </template>
              </el-table-column>
            </el-table>
          </el-collapse-item>
          ...
          <el-collapse-item title="普通mock" name="4">
            <el-card class="box-card">
              ...
              <el-input v-model.trim="form.mockData" type="textarea" :rows="8" placeholder="请输入mock内容" />
            </el-card>
          </el-collapse-item>
        </el-collapse>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    getApiInfo() {
      const self = this
      const param = { project_id: self.$route.params.project_id, api_id: self.$route.params.api_id }
      $.ajax({
        type: 'get',
          ...
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            if (self.parameterRaw && _type === 'raw') {
              if (!self.isJsonString(self.parameterRaw)) {
                ...
                // console.logs(_parameter)
                // console.logs(typeof _parameter)
                $.ajax({
                  type: 'post',
