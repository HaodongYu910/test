<template>
    <section>
        <router-link :to="{ name: '用例接口列表', params: {project_id: this.project_id,case_id: this.$route.params.case_id}}" style='text-decoration: none;color: aliceblue;'>
    ...
    export default {
        ...
        methods: {
            ...
            handleCorrelation(index, row) {
                ...
                $.ajax({
                    type: "get",
                    url: test+"/Interface/automation/get_correlation_response",
                    async: true,
                ...
                this.$refs.form.validate((valid) => {
                    if (valid) {
                        ...
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            $.ajax({
                                type: "post",
                                url: test+"/Interface/automation/add_new_api",
                                async: true,
                                data: param,
                                headers: {
                                    Authorization: 'Token '+JSON.parse(sessionStorage.getItem('token'))
