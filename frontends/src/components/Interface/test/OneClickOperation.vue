    ...
    methods: {
      ...
      checkName(callback) {
        for (let i of this.selectProjectId) {
          this.result = this.$post('/Interface/checkName', {name: this.ruleForm.testName, projectId: i}, () => {
            if (callback) callback();
          })
        }
      },
      _getEnvironmentAndRunTest: function (item) {
        this.result = this.$get('/Interface/environment/list/' + item.projectId, response => {
      ...
      save(callback) {
        this.change = false;
        let url = "/Interface/create/merge";
