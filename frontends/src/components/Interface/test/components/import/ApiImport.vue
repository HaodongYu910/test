      ...
      methods: {
        ...
        getEnvironments() {
          if (this.formData.projectId) {
            this.$get('/Interface/environment/list/' + this.formData.projectId, response => {
        ...
        save() {
          this.$refs.form.validate(valid => {
            if (valid) {
              ...
              this.result = this.$fileUpload('/Interface/import', param.file, null, param,response => {
                let res = response.data;
                this.$success(this.$t('test_track.case.import.success'));
                this.visible = false;
                this.$router.push({path: '/Interface/test/edit', query: {id: res.id}});
              });
            } else {
              return false;
            }
