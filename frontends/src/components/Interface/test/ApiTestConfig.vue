  ...
  export default {
    ...
    methods: {
      ...
      updateReference() {
        ...
        // 更新引用场景
        this.result = this.$post("/Interface/list/ids", {ids: updateIds}, response => {
          let scenarioMap = {};
          if (response.data) {
      ...
      getTest(id) {
        this.result = this.$get("/Interface/get/" + id, response => {
          if (response.data) {
      ...
      save(callback) {
        ...
        let url = this.create ? "/Interface/create" : "/Interface/update";
      ...
      saveTest() {
        this.save(() => {
          ...
          if (this.create) {
            this.$router.push({
              path: '/Interface/test/edit?id=' + this.test.id
            })
          }
          // 发送广播，刷新 head 上的最新列表
      ...
      runTest() {
        this.result = this.$post("/Interface/run", {id: this.test.id, triggerMode: 'MANUAL'}, (response) => {
          this.$success(this.$t('api_test.running'));
          this.$router.push({
            path: '/Interface/report/view/' + response.data
          })
        });
      },
      saveRunTest() {
      ...
      cancel() {
        this.$router.push('/Interface/test/list/all');
      },
      ...
      saveSchedule() {
        ...
        let url = '/Interface/schedule/create';
        if (param.id) {
          url = '/Interface/schedule/update';
        }
        this.$post(url, param, () => {
          this.$success(this.$t('commons.save_success'));
