...
export default {
  ...
  data() {
    return {
      ...
      triggerFilters: [
        ...
        {text: this.$t('commons.trigger_mode.Interface'), value: 'API'}
      ],
      buttons: [
        {
  ...
  methods: {
    search() {
      ...
      let url = "/Interface/report/list/" + this.currentPage + "/" + this.pageSize;
    ...
    handleView(report) {
      this.$router.push({
        path: '/Interface/report/view/' + report.id,
      })
    },
    handleDelete(report) {
      this.$alert(this.$t('api_report.delete_confirm') + report.name + "？", '', {
        ...
        callback: (action) => {
          if (action === 'confirm') {
            this.result = this.$post("/Interface/report/delete", {id: report.id}, () => {
      ...
      this.$alert(this.$t('api_report.delete_batch_confirm') + "？", '', {
        ...
        callback: (action) => {
          if (action === 'confirm') {
            ...
            this.$post('/Interface/report/batch/delete', {ids: ids}, () => {
              this.selectRows.clear();
              this.$success(this.$t('commons.delete_success'));
              this.search();
              // 发送广播，刷新 head 上的最新列表
