        ...
        <div class="filter-container">
            ...
            <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
                <el-form :inline="true" :model="filters" @submit.native.prevent>
                    <el-form-item>
                        <el-input v-model="filters.patientname" placeholder="patientname"
                                  @keyup.enter.native="getDurationlist"/>
                    </el-form-item>
            ...
            <el-table
                ...
                <el-table-column prop="预测结果" label="预测结果" min-width="9%">
                    <template slot-scope="scope">
                        <span style="margin-left: 10px">{{ state(scope.row.aistatus,"ai") }}</span>
                    </template>
                </el-table-column>
                ...
                <!--            <template slot-scope="scope">-->
                <!--              <span style="margin-left: 12px">{{ scope.row.update_time  | dateformat('YYYY-MM-DD HH:MM:SS') }}</span>-->
                <!--            </template>-->
                <!--          </el-table-column>-->
                <el-table-column prop="发送状态" label="发送状态" min-width="20%">
                    <template slot-scope="scope">
                        <span style="margin-left: 10px" :class="cssStyle(scope.row.status)">{{state(scope.row.status,'send')}}</span>
                    </template>
                </el-table-column>
            ...
            <el-dialog title="源数据" :visible.sync="sourceFormVisible" :close-on-click-modal="false"
                ...
                <div slot="footer" class="dialog-footer">
                    ...
                    <el-button type="primary" @click.native="handleDisable(sourceForm.studyinstanceuid)"
                               :loading="sourceLoading">禁用
                    </el-button>
                </div>
            </el-dialog>
        </div>
