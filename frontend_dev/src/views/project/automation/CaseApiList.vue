<template>
  <section>
    <router-link :to="{ name: '用例列表', params: {project_id: this.$route.params.project_id}}" style="text-decoration: none;color: aliceblue;">
      <el-button class="return-list"><i class="el-icon-d-arrow-left" style="margin-right: 5px" />用例列表</el-button>
    </router-link>
    <el-button type="primary" @click.native="addOldApi"><i class="el-icon-plus" style="margin-right: 5px" />已有接口</el-button>
    <router-link :to="{ name: '添加新接口'}" style="text-decoration: none;color: #000000;">
      <el-button type="primary"><i class="el-icon-plus" style="margin-right: 5px" />新建接口</el-button>
    </router-link>
    ...
    <el-dialog title="选择创建的API" :visible.sync="searchApiListVisible" :close-on-click-modal="false">
        ...
        <el-col :span="6">
          <div style="height:400px;line-height:100px;overflow:auto;overflow-x:hidden;border: 1px solid #e6e6e6">
            <el-menu
              ...
              @select="handleSelect"
            >
              <el-menu-item index="-1" @click.native="getApiList()"><i class="el-icon-menu" />所有接口</el-menu-item>
              <template v-for="(item,index) in groupData">
                <el-menu-item :key="item.id" :index="index+''" class="group" @click.native="getApiList(item.id)">
      ...
      <el-col :span="24" class="toolbar">
        <el-pagination layout="prev, pager, next" :page-size="20" :page-count="total" style="float:right;" @current-change="handleCurrentChangeApi" />
      </el-col>
      <div slot="footer" class="dialog-footer">
        <el-button @click.native="searchApiListVisible = false">取消</el-button>
        <el-button type="primary" :loading="searchApi" @click.native="addOldApiSubmit">提交</el-button>
      </div>
    </el-dialog>
    <el-table v-loading="listLoading" :data="ApiList" highlight-current-row style="width: 100%;">
...
export default {
  ...
  methods: {
    ...
    getApiList(_list) {
      ...
      const self = this
      const param = { project_id: Number(this.$route.params.project_id), page: self.page }
      if (_list) {
