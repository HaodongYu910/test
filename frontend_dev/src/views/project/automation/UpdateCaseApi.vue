<template>
  <section>
    <router-link :to="{ name: '用例接口列表', params: {project_id: this.$route.params.project_id,case_id: this.$route.params.case_id}}" style="text-decoration: none;color: aliceblue;">
      <el-button class="return-list"><i class="el-icon-d-arrow-left" style="margin-right: 5px" />返回列表</el-button>
    </router-link>
    <router-link :to="{ name: '用例接口列表', params: {project_id: this.$route.params.project_id,case_id: this.$route.params.case_id}}" style="text-decoration: none;color: aliceblue;">
      ...
      <el-row :span="24">
        <el-collapse v-model="activeNames" @change="handleChange">
            ...
            <el-table :data="form.head" highlight-current-row>
              <el-table-column prop="name" label="标签" min-width="28%" sortable>
                <template slot-scope="scope">
                  <el-select v-model="scope.row.name" placeholder="head标签" filterable style="width: 90%">
              ...
              <el-table-column min-width="7%">
                <template slot-scope="scope">
                  <el-button v-show="scope.row.interrelate" type="primary" size="mini" style="margin-bottom: 5px" @click="handleCorrelation(scope.$index, scope.row)">关联</el-button>
                </template>
              </el-table-column>
          ...
          <el-collapse-item title="请求参数" name="2">
            ...
            <template>
              <el-input v-model.trim="form.parameterRaw" :class="ParameterTyep? 'parameter-b': 'parameter-a'" type="textarea" :rows="5" placeholder="请输入内容" />
            </template>
          </el-collapse-item>
          <el-dialog title="关联" :visible.sync="searchApiVisible" :close-on-click-modal="false">
              ...
              <el-col :span="6">
                <div style="height:400px;line-height:100px;overflow:auto;overflow-x:hidden;border: 1px solid #e6e6e6">
                  <el-menu default-active="2" class="el-menu-vertical-demo" active-text-color="rgb(32, 160, 255)" :unique-opened="true">
                    <el-menu-item v-for="(item,index) in ApiList" :key="item.id" :index="index+''" @click.native="handleResponse(index)">{{ item.name }}</el-menu-item>
                  </el-menu>
                </div>
              </el-col>
              <el-col :span="18">
            ...
            <div slot="footer" class="dialog-footer">
              ...
              <el-button type="primary" :loading="saveCorrelation" @click.native="addInterrelateSubmit">保存</el-button>
            </div>
          </el-dialog>
          <el-collapse-item title="测试结果校验" name="3">
            <el-card class="box-card">
              ...
              <div v-show="showCheck">
                ...
                <el-input v-model.trim="form.checkData" style="margin-top: 10px" type="textarea" :rows="8" placeholder="请输入mock内容" />
              </div>
            </el-card>
          </el-collapse-item>
        </el-collapse>
...
export default {
  ...
  methods: {
        ...
        success: function(data) {
          if (data.code === '0') {
            if (data.data.length) {
              ...
              // console.logs(self.interrelateObjects)
            } else {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          this.$confirm('确认提交吗？', '提示', {}).then(() => {
            ...
            const _type = self.form.parameterType
            let _parameter = {}
            if (_type === 'form-data') {
    ...
    addHead() {
      const headers = { name: '', value: '', interrelate: 0 }
