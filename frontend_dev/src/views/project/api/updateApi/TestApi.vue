    ...
    <el-form ref="form" :model="form" :rules="formRules">
      ...
      <div style="border: 1px solid #e6e6e6;margin-bottom: 10px; padding:15px; padding-bottom: 0px">
        <el-row :gutter="10">
          ...
          <el-col :span="2">
            <el-button type="primary" :loading="loadingSend" @click="Test">发送</el-button>
          </el-col>
        </el-row>
      </div>
      <el-row :span="24">
        <el-collapse v-model="activeNames" @change="handleChange">
          <el-collapse-item title="请求头部" name="1">
            <el-table ref="multipleHeadTable" :data="form.head" highlight-current-row>
          ...
          <el-collapse-item title="响应结果" name="4">
            ...
            <el-card class="box-card">
              ...
              <div v-show="!format" v-model="form.resultData" :class="resultShow? 'parameter-a': 'parameter-b'">
          ...
          <el-collapse-item title="请求历史" name="5">
            <el-table v-loading="listLoading" :data="requestHistory" stripe style="width: 100%">
              ...
              <el-table-column min-width="10%" label="操作">
                <template slot-scope="scope">
                  <i class="el-icon-delete" style="font-size:30px;cursor:pointer;" @click="delHistory(scope.row)" />
                </template>
              </el-table-column>
            </el-table>
          </el-collapse-item>
...
export default {
  ...
  methods: {
    ...
    AddHistroy(code) {
      const self = this
      this.listLoading = true
      const param = JSON.stringify({ project_id: Number(this.$route.params.project_id),
    ...
    Test: function() {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          console.log(headers)
          const url = self.form.Http4 + '://' + self.form.url + host
          const _type = self.radio
          if (_type === 'form-data') {
            if (self.radioType) {
    ...
    neatenFormat() {
      const demo = document.getElementsByTagName('pre')[0]
      console.log(demo)
    ...
    addParameter() {
      const headers = { name: '', value: '', required: 'True', restrict: '', description: '' }
      this.form.parameter.push(headers)
      const rows = [this.form.parameter[this.form.parameter.length - 1]]
