  ...
  <section>
    <el-row :span="24" class="row-title">
          ...
          <el-menu default-active="2" class="el-menu-vertical-demo" active-text-color="rgb(32, 160, 255)" :unique-opened="true">
            <template v-for="(item,index) in groupData">
              <router-link :to="{ name: '分组接口列表', params: {project_id: project, firstGroup: item.id}}" style="text-decoration:none;">
                <el-menu-item :key="item.id" :index="index+''" class="group">
      ...
      <el-dialog title="新增分组" :visible.sync="addGroupFormVisible" :close-on-click-modal="false" style="width: 60%; left: 20%">
        <el-form ref="addGroupForm" :model="addGroupForm" label-width="80px" :rules="addGroupFormRules">
          <el-form-item label="分组名称" prop="firstgroup">
            <el-input v-model.trim="addGroupForm.firstgroup" auto-complete="off" style="width: 90%" />
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click.native="addGroupFormVisible = false">取消</el-button>
          <el-button type="primary" :loading="addGroupLoading" @click.native="addGroupSubmit">提交</el-button>
        </div>
      </el-dialog>
      <!--编辑父分组-->
      <el-dialog title="编辑分组" :visible.sync="editFirstGroupFormVisible" :close-on-click-modal="false" style="width: 60%; left: 20%">
        <el-form ref="editFirstGroupForm" :model="editFirstGroupForm" label-width="80px" :rules="editFirstGroupFormRules">
          <el-form-item label="分组名称" prop="secondFirstGroup">
            <el-input v-model.trim="editFirstGroupForm.secondFirstGroup" auto-complete="off" />
          </el-form-item>
        </el-form>
        <div slot="footer" class="dialog-footer">
          <el-button @click.native="editFirstGroupFormVisible = false">取消</el-button>
          <el-button type="primary" :loading="editFirstGroupLoading" @click.native="editFirstGroupSubmit">提交</el-button>
        </div>
      </el-dialog>
      <el-col :span="20">
...
<script>
import { addApiGroup, delApiGroup, getApiGroupList, updateApiGroup } from '../@/router/api'
export default {
  data() {
    return {
      project: '',
