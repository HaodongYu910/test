<template>
  <section>
    <router-link :to="{ name: '接口列表', params: {project_id: this.$route.params.project_id}}" style="text-decoration: none;color: aliceblue;">
      <el-button class="return-list"><i class="el-icon-d-arrow-left" style="margin-right: 5px" />接口列表</el-button>
    </router-link>
    <router-link :to="{ name: '接口列表', params: {project_id: this.$route.params.project_id}}" style="text-decoration: none;color: aliceblue;">
        ...
        <el-row :gutter="10">
          <el-col :span="8">
            <el-form-item label="接口名称:" label-width="83px" prop="name">
              <el-input v-model.trim="form.name" placeholder="名称" auto-complete />
            </el-form-item>
          </el-col>
          <el-col :span="10">
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="18">
            <el-form-item prop="apiAddress">
              <el-input v-model.trim="form.apiAddress" placeholder="地址" auto-complete />
            </el-form-item>
          </el-col>
        </el-row>
      </div>
        ...
        <el-collapse v-model="activeNames" @change="handleChange">
            ...
            <el-table :data="form.headDict" highlight-current-row>
              <el-table-column prop="name" label="标签" min-width="20%" sortable>
                <template slot-scope="scope">
                  <el-select v-model="scope.row.name" placeholder="head标签" filterable>
                    <el-option v-for="(item,index) in header" :key="index+''" :label="item.label" :value="item.value" />
                  </el-select>
                  <el-input v-model.trim="scope.row.name" class="selectInput" :value="scope.row.name" placeholder="请输入内容" />
                </template>
              </el-table-column>
              <el-table-column prop="value" label="内容" min-width="40%" sortable>
                <template slot-scope="scope">
                  <el-input v-model.trim="scope.row.value" :value="scope.row.value" placeholder="请输入内容" />
                </template>
              </el-table-column>
              <el-table-column label="操作" min-width="10%">
                <template slot-scope="scope">
                  <i class="el-icon-delete" style="font-size:30px;cursor:pointer;" @click="delHead(scope.$index)" />
                </template>
              </el-table-column>
              <el-table-column label="" min-width="10%">
                <template slot-scope="scope">
                  <el-button v-if="scope.$index===(form.headDict.length-1)" size="mini" class="el-icon-plus" @click="addHead" />
                </template>
              </el-table-column>
            </el-table>
          </el-collapse-item>
          <el-collapse-item title="请求参数" name="2">
            <div style="margin: 5px">
              <el-row :span="24">
                ...
                <el-col v-if="request3" :span="16"><el-checkbox v-show="ParameterTyep" v-model="radioType" label="3">表单转源数据</el-checkbox></el-col>
              </el-row>
            </div>
            <el-table :data="form.requestList" highlight-current-row :class="ParameterTyep? 'parameter-a': 'parameter-b'">
              ...
              <el-table-column prop="_type" label="参数类型" min-width="14%" sortable>
                <template slot-scope="scope">
                  <el-select v-model="scope.row._type" placeholder="请求方式">
                    <el-option v-for="(item,index) in paramTyep" :key="index+''" :label="item.label" :value="item.value" />
                  </el-select>
                </template>
              </el-table-column>
              <el-table-column prop="description" label="参数说明" min-width="14%" sortable>
                <template slot-scope="scope">
                  <el-input v-model.trim="scope.row.description" :value="scope.row.desc" placeholder="请输入参数说明" />
                </template>
              </el-table-column>
              <el-table-column label="操作" min-width="13%">
                <template slot-scope="scope">
                  <i class="el-icon-delete" style="font-size:30px;cursor:pointer;" @click="delParameter(scope.$index)" />
              ...
              <el-table-column label="" min-width="4%">
                <template slot-scope="scope">
                  <el-button v-if="scope.$index===(form.requestList.length-1)" size="mini" class="el-icon-plus" @click="addParameter" />
                </template>
              </el-table-column>
            </el-table>
            <template>
              <el-input v-model.trim="parameterRaw" :class="ParameterTyep? 'parameter-b': 'parameter-a'" type="textarea" :rows="5" placeholder="请输入内容" />
            </template>
          </el-collapse-item>
          <el-dialog title="更多设置" :visible.sync="addParameterFormVisible" :close-on-click-modal="false">
            <el-form ref="editForm" :model="editForm" label-width="60px" :rules="FormRules">
              <el-form-item label="参数名" label-width="83px">
                <el-input v-model.trim="editForm.name" auto-complete="off" placeholder="请输入参数名称" />
              </el-form-item>
              <el-form-item label="参数值" label-width="83px">
                <el-input v-model.trim="editForm.value" auto-complete="off" placeholder="请输入参数值" />
              </el-form-item>
              <el-form-item label="必填?" label-width="83px" prop="required">
                <el-select v-model="editForm.required" placeholder="必填？">
                  <el-option v-for="(item,index) in required4" :key="index+''" :label="item.label" :value="item.value" />
                </el-select>
              </el-form-item>
              <el-form-item label="输入限制" prop="version" label-width="83px">
                <el-input v-model.trim="editForm.restrict" auto-complete="off" placeholder="请输入输入限制" />
              </el-form-item>
              <el-form-item label="描述" prop="description" label-width="83px">
                <el-input v-model.trim="editForm.description" type="textarea" :rows="7" placeholder="请输入描述" />
              </el-form-item>
            </el-form>
          ...
          <el-collapse-item title="返回参数" name="3">
            <el-table :data="form.responseList" highlight-current-row>
              ...
              <el-table-column label="操作" min-width="13%">
                <template slot-scope="scope">
                  <i class="el-icon-delete" style="font-size:30px;cursor:pointer;" @click="delResponse(scope.$index)" />
                  <el-button type="primary" size="mini" style="margin-bottom: 5px" @click="handleResponseEdit(scope.$index, scope.row)">更多设置</el-button>
                </template>
              </el-table-column>
              <el-table-column label="" min-width="4%">
                <template slot-scope="scope">
                  <el-button v-if="scope.$index===(form.responseList.length-1)" size="mini" class="el-icon-plus" @click="addResponse" />
                </template>
              </el-table-column>
            </el-table>
          </el-collapse-item>
          ...
          <el-collapse-item title="普通mock" name="4">
            <el-card class="box-card">
              ...
              <el-input v-model.trim="form.data" type="textarea" :rows="8" placeholder="请输入mock内容" />
            </el-card>
          </el-collapse-item>
        </el-collapse>
      </el-row>
...
<script>
import { addApiDetail, getApiGroupList } from '../@/router/api'
...
export default {
  ...
  methods: {
    checkRequest() {
      const request = this.form.requestType
    ...
    addParameter() {
      const headers = { name: '', value: '', _type: 'String', required: true, restrict: '', description: '' }
    ...
    addResponse() {
      const headers = { name: '', value: '', _type: 'String', required: true, description: '' }
    ...
    fastAdd() {
      const form = this.$route.params.formData
      const _type = this.$route.params._type
      const _typeData = this.$route.params._typeData
      if (form) {
