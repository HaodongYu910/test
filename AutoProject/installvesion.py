        ...
        try:
            ...
            try:
                if self.Flag is True:
                    ...
                    sendMessage(touser='', toparty='132', message='【安装部署】：（{0}）下载最新安装包'.format(self.obj.Host.host))
                    AddJournal(name="Installation{}".format(self.id), content="【安装部署】：下载安装包\n")

                    self.ssh.cmd(
                        'mkdir install_version/models;mkdir install_version/engine;mkdir install_version/docker-images')
                    self.ssh.cmd(
                        "rclone copy oss://biomind-ha-rt/biomind-model-repository/{0} /home/biomind/install_version/models;" \
                        "rclone copy oss://biomind-ha-se/versions/{1} /home/biomind/install_version/engine;" \
                        "cp /home/biomind/install_version/engine/installer/setup_engine.sh /home/biomind/install_version/.".format(
                ...
                AddJournal(name="Installation{}".format(self.id),
                           content="【安装部署】：{0}版本安装包 失败原因：{1}".format(self.obj.version, e))
            ...
            try:
                ...
                AddJournal(name="Installation{}".format(self.id),
                           content="【安装部署】：安装{0}版本安装包失败原因：{1}".format(self.obj.version, e))
                self.installStatus(status=False, type=3)
                return
            try:
                ...
                AddJournal(name="Installation{}".format(self.id),
                           content="【安装部署】：安装{0}版本更新文件失败原因：{1}".format(self.obj.version, e))
                self.installStatus(status=False, type=3)
                return
            try:
