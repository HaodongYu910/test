...
from AutoProject.common.jiraData import Jiradata
...
#jira 数据图表
def data_figure(platform,sprint_version):
    name = str(re.findall(r"(.+?)-", platform)[0])
    if sprint_version=='':
        return False
    else:
        iris ="""<h4 style="color:#000000">%s创建与解决问题对比图</h4><img width="500"src="%s">
        <img width="500" src="%s">""" %(name,dataframe(platform, sprint_version),frame(platform, sprint_version)) # 绘制 创建与解决问题数据
        return iris
