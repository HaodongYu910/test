...
from AutoProject.utils.graphql.graphql_prediction_series import graphql_prediction_series
...
def get_graphql_result(study_uid,series,kc):
    ...
    logger.info('query study uid [{0}], result is: {1}'.format(study_uid, queryresult))
    logger.info('predict duration: {}'.format(duration))
        ...
        if len(queryresult['ai_biomind']['preport']) != 0:
            if queryresult['ai_biomind']['pprediction'] is not None:
                ...
                for volumeID, predictionList in predictions.items():
                    for predictionItem in predictionList:
                        ...
                        if 'disease' in predictionItem['category']:
                            ...
                            logger.info("clas_dict: {}".format(clas_dict))
                ...
                # clas_dict={}, no disease
                    ...
                    logger.info('ai_status_result: {}'.format(ai_status_result))
