...
def test():
    ...
    for j in range(20):
        ...
        for i in vop_data["data"]["data"]:
            print("用户",i["user"]["nickname"],"实盘收益率",i["recommend"]["current_incr_ratio"],"推荐币成功率",i["user"]["success_ratio"],"推荐币收益率",i["user"]["realised_pnl_ratio"])
...
def testpost():
    for i in range(50):
        ...
        for j in vop_data["data"]["data"]["allKolInfo"]["resultList"]:
            print(vop_data["data"]["data"]["allKolInfo"]["resultList"][a]["kolname"],"￥"+str(vop_data["data"]["data"]["allKolInfo"]["resultList"][0]["balance"])+"元")
            a=a+1
